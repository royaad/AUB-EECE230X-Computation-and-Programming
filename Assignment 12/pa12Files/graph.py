...
class DiGraph(object):
    ...
    # List of nodes could be generated via: list(self.adj.keys()) 
    def __init__(self):
        self.adj = {}
    def addNode(self,u):
        """ add node u """
    ...
    def connect(self,u,v):
        ...
        self.adj[u].append(v)
    def __str__(self):
        """ represent the  graph as a string:  for each node u, 
            the string has a  line  of the form:  
            u: adjacent nodes separated commas"""
        s = ''
        for u in self.adj:
            s = s+str(u)+' : '+str(self.adj[u])[1:-1]+'\n'
        return s
    def draw(self):
